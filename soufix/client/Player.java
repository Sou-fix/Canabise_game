...
public class Player
{
  ...
  private String stuff_lo_load;
  private String sell_lo_load;
  ...
  public Player(int id, String name, int groupe, int sexe, int classe, int color1, int color2, int color3, long kamas, int pts, int _capital, int energy, int level, long exp, int _size, int _gfxid, byte alignement, int account, Map<Integer, Integer> stats, byte seeFriend, byte seeAlign, byte seeSeller, String canaux, short map, int cell, String stuff, String storeObjets, String pdvPer, String spells, String savePos, String jobs, int mountXp, int mount, int honor, int deshonor, int alvl, String z, byte title, int wifeGuid, String morphMode, String allTitle, String emotes, long prison, boolean isNew, String parcho, long timeDeblo, boolean noall, String deadInformation, byte deathCount, long totalKills, final int tokens, final int apExo, final int mpExo, final int raExo,String wi)
  {
    ...
    this.stuff_lo_load=stuff;
    this.sell_lo_load=storeObjets;
    try
    {
  ...
  public String parseALK()
  {
    ...
    perso.append(this.WornItem).append(";");
  ...
  public void OnJoinGame()
  {
    ...
    if(this.account.getGameClient()==null)
      return;
    this.account.load_items();
    if(!this.stuff_lo_load.equals(""))
    {
      if(this.stuff_lo_load.charAt(this.stuff_lo_load.length()-1)=='|')
    	  this.stuff_lo_load=this.stuff_lo_load.substring(0,this.stuff_lo_load.length()-1);
      Database.getStatics().getObjectData().load(this.stuff_lo_load.replace("|",","));
    }
    for(String item : this.stuff_lo_load.split("\\|"))
    {
      if(item.equals(""))
        continue;
      String[] infos=item.split(":");
    ...
    if(!this.sell_lo_load.equals(""))
    {
      for(String _storeObjets : this.sell_lo_load.split("\\|"))
      {
    ...
    if(this.getCurMap().getSubArea()!=null)
    {
      if(this.getCurMap().getSubArea().getId()==319||this.getCurMap().getSubArea().getId()==210)
        new TimerWaiterPlus(() -> Minotoror.sendPacketMap(this),3000);
      else if(this.getCurMap().getSubArea().getId()==200)
        new TimerWaiterPlus(() -> PigDragon.sendPacketMap(this),3000);
    }
    if(this.getEnergy()==0)
      this.setGhost();
    ...
    this.sendMessage("Bienvenue sur <b>Ravens</b>, il y a actuellement <b>"+Main.world.getOnlinePlayers().size()+"</b> joueurs en ligne.");
    Main.refreshTitle();
    this.verifEquiped();
	  }
	    catch(Exception e)
  ...
  public String parseToGM()
  {
    ...
    if(fight==null&&curCell!=null)// Hors combat
    {
      ...
      if(this.hasEquiped(10054)||this.hasEquiped(10055)||this.hasEquiped(10056)||this.hasEquiped(10058)||this.hasEquiped(10061)||this.hasEquiped(10102))
      {
        str.append(3).append(";");
        this.set_title(2);
        this.setAllTitle(String.valueOf(2));
      }
      else
      {
        if(get_title()==2)
        {
          this.set_title(0);
          this.setAllTitle(String.valueOf(0));
        }
  ...
  public String getGMStuffString()
  {
    ...
    catch(Exception e){ return this.WornItem ==null ? ",,,," : this.WornItem;};
    return str.toString();
  }

  public String getAsPacket(boolean boutique)
  {
    refreshStats();
    refreshLife(true);
  ...
  public Stats getStuffStats()
  {
    ...
    synchronized (this.objects.values()) {
  ...
  public int getMaxPod()
  {
    ...
    if(this.getAccount().getSubscribeRemaining() != 0L)pods+=10000;
  ...
  public String parseObjetsToDB()
  {
    StringBuilder str=new StringBuilder();
    if(objects.isEmpty()) {
     if(this.stuff_lo_load == "" || this.stuff_lo_load == null) {
      return "";
     }else
    	 return this.stuff_lo_load;
  ...
  public void addObjet(GameObject newObj)
  {
    ...
    if(newObj!=null)
    {
      if(newObj.modification==0)
        Database.getStatics().getObjectData().insert(newObj);
      else if(newObj.modification==1)
        Database.getStatics().getObjectData().update(newObj);
    }
  }
  ...
  public void teleport(short newMapID, int newCellID)
  {
    ...
    if(map.getSubArea()!=null)
    {
      ...
      {
        new TimerWaiterPlus(() -> PigDragon.sendPacketMap(this),1000);
      }
      else if(map.getSubArea().getId()==210||map.getSubArea().getId()==319)
      {
        new TimerWaiterPlus(() -> Minotoror.sendPacketMap(this),1000);
      }
    }
  }
  ...
  public String getStringTitle(int title) { 
	  if(World.gettitre(title) != null) {
		return ""+World.gettitre(title).Color+"*"+World.gettitre(title).Content+"";
	  }else {
		  return ""+title;  
	  }
  ...
  public void addInBank(int guid, int qua)
  {
    ...
    if(BankObj==null) // Ajout d'un nouvel objet dans la banque
    {
      ...
      {
        PersoObj.setQuantity(newQua); //on modifie la quantitï¿½ d'item du sac
    ...
    // S'il y avait un item du meme template
    {
      if(newQua<=0) //S'il ne reste pas d'item dans le sac
      {
        ...
        BankObj.setQuantity(BankObj.getQuantity()+PersoObj.getQuantity()); //On ajoute la quantitï¿½ a l'objet en banque
  ...
  public void openMountPark()
  {
    ...
    //new TimerWaiterPlus(() -> mountPark.getEtable().stream().filter(mount -> mount!=null&&mount.getSize()==50&&mount.getOwner()==this.getId()).forEach(mount -> SocketManager.GAME_SEND_Ee_PACKET_WAIT(this,'~',mount.parse())),500);
  }
  ...
  public void resetVars()
  {
    if(this.getExchangeAction()!=null)
    {
      if(this.getExchangeAction().getValue() instanceof JobAction&&((JobAction)this.getExchangeAction().getValue()).getJobCraft()!=null)
        ((JobAction)this.getExchangeAction().getValue()).getJobCraft().jobAction.broken=true;
  ...
  {
    StringBuilder str=new StringBuilder();
   if(this.sell_lo_load == null || this.sell_lo_load == "") {
	 
   }else
	   return this.sell_lo_load;  
 ...
 /*
  ...
  public void resetStringStuff(){
		try{
			this.WornItem = this.getGMStuffString();
		}
		catch(Exception e){
			e.printStackTrace();
		}
