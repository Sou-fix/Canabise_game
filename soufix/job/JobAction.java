...
public class JobAction
{
  ...
  public void endAction(Player player, InteractiveObject IO, GameAction GA, GameCase cell)
  {
    ...
    if(!this.isCraft&&player.getGameClient().action!=0)
    {
      ...
      {
    	 new TimerWaiterPlus(() -> endAction(player,IO,GA,cell),3000);
        return;
      }
    }
    ...
    if(!this.isCraft)
    {
      ...
      int chance = SM.getTemplate().getId()==JobConstant.JOB_PECHEUR ? 20 : 40;
      if(Formulas.getRandomValue(1,250)==1)
      {
        int _tID=-1;
        int rareQua=1;
  ...
  public synchronized void craft(boolean repeat)
  {
    ...
    if(!isUnjobSkill)
    {
      ...
      if(Logging.USE_LOG)
          Logging.getInstance().write("SecureCraft",this.player.getName()+" ï¿½ crafter avec "+(success ? "SUCCES" : "ECHEC")+" l'item "+tID+" ("+Main.world.getObjTemplate(tID).getName()+")");
      if(!success)
      {
  ...
  public boolean craftPublicMode(Player crafter, Player receiver, Map<Player, ArrayList<Pair<Integer, Integer>>> list)
  {
    ...
    if(Logging.USE_LOG)
      Logging.getInstance().write("SecureCraft",this.player.getName()+" ï¿½ crafter avec "+(success ? "SUCCES" : "ECHEC")+" l'item "+template+" ("+Main.world.getObjTemplate(template).getName()+") pour "+receiver.getName());
    if(!success)
    {
      SocketManager.GAME_SEND_Ec_PACKET(this.player,"EF");
...
public boolean doMage(boolean repeat, Player receiver, Map<Player, ArrayList<Pair<Integer, Integer>>> items)
  {
    ...
    {
      SocketManager.GAME_SEND_MESSAGE(this.player,"Vous ne pouvez mage que deux fois le niveau de votre travail.");
      if(receiver!=null)
      {
    	  receiver.send("EMKO+"+runeObject.getGuid()+"|"+runeObject.getQuantity());
    	  receiver.send("EMKO+"+objectFm.getGuid()+"|"+objectFm.getQuantity());
        clearMage(objectFm,runeObject,receiver);
      }
      else
      {
    	  this.player.send("EMKO+"+runeObject.getGuid()+"|"+runeObject.getQuantity());
    	  this.player.send("EMKO+"+objectFm.getGuid()+"|"+objectFm.getQuantity());
   ...
   // System.out.println("exoPower: "+exoPower);
    ...
    else
    {
      ...
      //v2.8 - smarter max stat calculation
       if(potion == null)
       if(statMax == 0) {
    	  if (currentStat+rune.getStatsAdd() > 20)
    	  {
      ...
      {
        SocketManager.GAME_SEND_MESSAGE(this.player,"Ce mage dépasserait la limite de statistiques maximale autorisée "+maxOvermage+".");
    ...
    if(rune.getStatId().compareTo("80") == 0 || rune.getStatId().compareTo("6f") == 0) {
  	...
  	if(Formulas.getRandomValue(1, 3) == 2)
  		fm_exo = true;
  	}
  	if(getStatBaseMaxs(objectFm.getTemplate(),rune.getStatId()) == 0 && Job.getActualJet(objectFm,rune.getStatId()) == 0)  {
  		if(Formulas.getRandomValue(1, player.getAccount().getSubscribeRemaining() == 0L ? 20 : 30) == 11)
  	  		fm_exo = true;	
  	}
    	}else {
    		chances.add(0,0);
