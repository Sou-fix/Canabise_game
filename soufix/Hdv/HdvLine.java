...
public class HdvLine
{
  ...
  {
    return this.getStrStats().equalsIgnoreCase(toAdd.getGameObject().parseStatsStringSansUserObvi())&&toAdd.getGameObject().getTemplate().getType()!=85;//R�cup�re les stats de l'objet et compare avec ceux de la ligne
  }
  ...
  {
    int totalSize=this.getEntries().get(0).size()+this.getEntries().get(1).size()+this.getEntries().get(2).size();//Additionne le nombre d'objet de chaque quantit�
