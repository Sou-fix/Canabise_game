...
public GameMap(short id, String date, byte w, byte h, String key, String places, String dData, String monsters, String mapPos, byte maxGroup, byte fixSize, byte minSize, byte maxSize, String forbidden, byte sniffed, int minRespawnTime, int maxRespawnTime)
  {
    this.id=id;
    this.date=date;
    this.w=w;
  ...
  public synchronized String getGMsPackets()
  {
	  try {	  
  ...
  public synchronized String getFightersGMsPackets(Fight fight)
  {
	  try {	  
  ...
  public synchronized String getObjectsGDsPackets()
  {
    ...
    this.cases.stream().filter(gameCase -> gameCase.getObject()!=null).forEach(gameCase -> packet.append("|").append(gameCase.getId()).append(";").append(gameCase.getObject().getState()).append(";").append((gameCase.getObject().isInteractive() ? "1" : "0")));
    return packet.toString();
  }
  ...
  public void startFightVersusMonstres(Player perso, MobGroup group)
  {
    ...
    {
      new TimerWaiterPlus(() -> party.getPlayers().stream().filter((follower) -> party.isWithTheMaster(follower,false)).forEach(follower -> {
    ...
    try
    {
      ...
      while(cantGruposAMover<mover)
      {
        ...
        for(final MobGroup grupoMob : mobGroups.values())
        {
          ...
          if(this.getId() == 534 || this.getId() == 2021 || this.getId() == 11095)
          continue;
  ...
  private void spawnZilla()
  {
	...
	{
		SocketManager.SEND_Im1223_ALL("La porte du kralamour vient d'être ouverte Faîtes vite elle sera ouverture que pendant 1 heure.");
