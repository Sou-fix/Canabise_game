...
public class SpellEffect
{
        ...
        switch(id)
        {
          case 1018: //earth health% damage
          {
            ...
            else if(target.hasBuff(781)) //Jinx
            {
              ...
              dmg=Formulas.getRandomJet(args[5]);//%age de pdv inflig�

            int val=(int)Math.floor(((double)caster.getPdv()/100)*dmg);//Valeur des d�gats
            ...
            int finalDommage=applyOnHitBuffs(val,target,caster,fight,Constant.ELEMENT_TERRE);
          ...
          case 1019: //fire health% damage
          {
            ...
            int finalDommage=applyOnHitBuffs(val,target,caster,fight,Constant.ELEMENT_FEU);
            if(finalDommage>target.getPdv())
              finalDommage=target.getPdv();//Target va mourrir
            target.removePdv(caster,finalDommage);
            target.removePdvMax((int)Math.floor(finalDommage*(Config.getInstance().erosion+caster.getTotalStats().getEffect(Constant.STATS_ADD_ERO)-caster.getTotalStats().getEffect(Constant.STATS_REM_ERO)-target.getTotalStats().getEffect(Constant.STATS_ADD_R_ERO)+target.getTotalStats().getEffect(Constant.STATS_REM_R_ERO)))/100);
          ...
          case 1020: //water health% damage
          {
            ...
            int finalDommage=applyOnHitBuffs(val,target,caster,fight,Constant.ELEMENT_EAU);
            if(finalDommage>target.getPdv())
              finalDommage=target.getPdv();//Target va mourrir
            target.removePdv(caster,finalDommage);
            target.removePdvMax((int)Math.floor(finalDommage*(Config.getInstance().erosion+caster.getTotalStats().getEffect(Constant.STATS_ADD_ERO)-caster.getTotalStats().getEffect(Constant.STATS_REM_ERO)-target.getTotalStats().getEffect(Constant.STATS_ADD_R_ERO)+target.getTotalStats().getEffect(Constant.STATS_REM_R_ERO)))/100);
          ...
          case 1021: //air health% damage
          {
            ...
            int finalDommage=applyOnHitBuffs(val,target,caster,fight,Constant.ELEMENT_AIR);
            if(finalDommage>target.getPdv())
              finalDommage=target.getPdv();//Target va mourrir
            target.removePdv(caster,finalDommage);
            target.removePdvMax((int)Math.floor(finalDommage*(Config.getInstance().erosion+caster.getTotalStats().getEffect(Constant.STATS_ADD_ERO)-caster.getTotalStats().getEffect(Constant.STATS_REM_ERO)-target.getTotalStats().getEffect(Constant.STATS_ADD_R_ERO)+target.getTotalStats().getEffect(Constant.STATS_REM_R_ERO)))/100);
  ...
  private void applyEffect_4(Fight fight, ArrayList<Fighter> cibles)
  {
   ...
   if(!caster.haveState(7))
    if(cell.isWalkable(false)&&!fight.isOccuped(cell.getId()))//Si la case est prise, on va �viter que les joueurs se montent dessus *-*
    {
    ...
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        else if(target.hasBuff(781)) //Jinx
        {
          ...
          dmg=Formulas.getRandomJet(args.split(";")[5]);//%age de pdv inflig�
  ...
  private void applyEffect_89(ArrayList<Fighter> cibles, Fight fight)
  {
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        int finalDommage=applyOnHitBuffs(val,target,caster,fight,Constant.ELEMENT_NEUTRE);
  ...
  private void applyEffect_90(ArrayList<Fighter> cibles, Fight fight)
  {
    if(turns<=0)//Si Direct
    {
      ...
      for(Fighter target : cibles)
      {
        ...
        {
          target.removePdv(caster,-(target.getPdvMax()-target.getPdv()));
          SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight,7,100,caster.getId()+"",target.getId()+",+"+(target.getPdvMax()-target.getPdv()));
        }
  ...
  private void applyEffect_120(ArrayList<Fighter> cibles, Fight fight)//Bonus PA
  {
    ...
    if(turns==0)
    {
    	caster.addBuff(Constant.STATS_REM_PA,val,1,1,true,spell,args,caster,false);
    }else {
    caster.addBuff(effectID,val,turns,1,true,spell,args,caster,false);
    }
  ...
  private void applyEffect_1003(ArrayList<Fighter> cibles, Fight fight)
  {
    ...
    for(Fighter target : cibles)
    {
      ...
      int dmg=33; //%age de pdv inflig�
      int val=(int)Math.floor((casterPdv/(float)100*(float)dmg)); //Valeur des d�gats
      val-=resF; //retrait de la r�sist fixe
