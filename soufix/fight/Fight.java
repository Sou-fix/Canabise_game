...
public class Fight
{
  ...
  public Fight(int type, int id, GameMap map, Player perso, Player init2)
  {
    ...
    if(perso.getGroupe().getId() > 1 && perso.getGroupe().getId() < 5) {
        STAFF = true;	
        }
  ...
  public Fight(int id, GameMap map, Player perso, MobGroup group)
  {
    ...
    for(Entry<Integer, MobGrade> entry : group.getMobs().entrySet())
    {
      ...
      if(entry.getValue().getTemplate().getId()==832) // Dï¿½minoboule
        Minotoror.demi();
  ...
  public static void FightStateAddFlag(GameMap map, Player player)
  {
    map.getFights().stream().filter(fight -> fight.state==Constant.FIGHT_STATE_PLACE).forEach(fight -> {
      ...
      else if(fight.type==Constant.FIGHT_TYPE_STAKE)
      {
        SocketManager.GAME_SEND_GAME_ADDFLAG_PACKET_TO_PLAYER(player,fight.init0.getPersonnage().getCurMap(),0,fight.init0.getId(),fight.init1.getId(),fight.init0.getPersonnage().getCurCell().getId(),"0;-1",fight.init1.getPersonnage().getCurCell().getId(),"0;-1");
        SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_PLAYER(player,fight.init0.getPersonnage().getCurMap(),fight.init0.getId(),fight.init0);
        SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_PLAYER(player,fight.init1.getPersonnage().getCurMap(),fight.init1.getId(),fight.init1);
  ...
  public void startFight()
  {
    ...
    if(this.collector!=null&&!this.collectorProtect)
    {
      ...
      for(Player player : protectors)
      {
        if(player.getFight()==null&&!player.isAway())
        {
          ...
          new TimerWaiterPlus(() -> this.joinCollectorFight(player,collector.getId()),1000);
        }
        else
        {
    ...
    if(getType()==Constant.FIGHT_TYPE_PVM||getType()==Constant.FIGHT_TYPE_DOPEUL)
    {
      ...
      String boss="58 85 86 107 113 121 147 173 180 225 226 230 232 251 252 257 289 295 374 375 377 382 404 423 430 457 478 568 605 612 669 670"+" 673 675 677 681 780 792 797 799 800 827 854 926 939 940 943 1015 1027 1045 1051 1071 1072 1085 1086 1087 1159 1184 1185 1186 1187 1188";
try {
  ...
  public void leftFight(Player playerCaster, Player playerTarget)
  {
    ...
    if(caster!=null)
    {
      switch(getType())
      {
        case Constant.FIGHT_TYPE_STAKE:
                        ...
                        if(getType()==Constant.FIGHT_TYPE_AGRESSION)
                        {
                          for(Fighter enemy : (this.getTeam1().containsValue(caster) ? this.getTeam0() : this.getTeam1()).values())
                          {
                            if(enemy.getPersonnage()!=null)
                            {
                              ...
                              {
                                new TimerWaiterPlus(() ->  player.teleportFaction(enemy.getPersonnage().get_align()),1500);
                                break;
                              }
                            }
                          }
                      ...
                      else
                      {
                        ...
                        else
                        {
                          if(player.isOnline())
                          {
                            ...
                            new TimerWaiterPlus(() ->  player.teleport(Short.parseShort(split[0]),Integer.parseInt(split[1])),1500);
                          }
                          else
                          {
  ...
  public void endTurn(final boolean onAction)
  {
    ...
    try
    {
      ...
      {
        new TimerWaiterPlus(() -> this.endTurn(onAction,current),100);
        return;
      }
      if(current.getPersonnage() != null){
			if (current.chamkar > (long)(System.currentTimeMillis()/1000)) {
				 new TimerWaiterPlus(() -> this.endTurn(onAction,current),2000);
				return;
			}
			}
      ...
      if(onAction)
        new TimerWaiterPlus(() -> this.newTurn(current),2100);
      else
  ...
  public void joinFight(Player perso, int guid)
  {
    ...
    if(getType()==Constant.FIGHT_TYPE_STAKE)
    {
      SocketManager.GAME_SEND_MESSAGE(perso,"You can not join this fight as it is forbidden to join a staking fight.");
      return;
    }
    if(perso.getGroupe() != null)
    if(perso.getGroupe().getId() > 1 && perso.getGroupe().getId() < 5) {
    ...
    if(getTeam0().containsKey(guid))
    {
      ...
      if(cell==null)
        return;
      if(getType()==Constant.FIGHT_TYPE_AGRESSION&&!Config.getInstance().HEROIC)
      {
    ...
    else if(getTeam1().containsKey(guid))
    {
      ...
      if(getType()==Constant.FIGHT_TYPE_AGRESSION&&!Config.getInstance().HEROIC)
      {
        ...
        if(multiIp)
        {
          SocketManager.GAME_SEND_MESSAGE(perso,"You can not join this fight, there is already a character in this fight with the same IP.");
          return;
        }
      }
      if(isOnlyGroup1())
  ...
  public void tryCaC(Player perso, int cellID)
  {
    ...
    else
    {
      ...
      boolean isEc=Formulas.isCriticalFail(arme.getTemplate().getTauxEC(),caster);
  ...
  public void onFighterDie(Fighter target, Fighter caster)
  {
    try
    {
      ...
      if(target.haveState(Constant.ETAT_PORTE))
        new TimerWaiterPlus(() -> removeCarry(target),3000); //timer so mob only gets dropped after already despawning
  ...
  public boolean playerReconnect(final Player perso)
  {
    ...
    else
    {
      ...
      SocketManager.GAME_SEND_GAMETURNSTART_PACKET(perso,current.getId(),(int)(System.currentTimeMillis()-launchTime));
      ...
      for(Fighter fighter : fighters)
      {
        ...
        if(!player.isFollowing)
        {
          if(Config.getInstance().teleports.containsKey((int)player.getCurMap().getId()))
          {
            ...
            new TimerWaiterPlus(() ->  player.teleport(player.getCurMap(),teleCell),1500);
          }
        }
  ...
  void onPlayerWin(Fighter fighter, ArrayList<Fighter> looseTeam)
  {
    ...
    new TimerWaiterPlus(() ->  player.teleportOld(),1500);
  ...
  void onPlayerLoose(Fighter fighter)
  {
    ...
    if(this.getType()!=Constant.FIGHT_TYPE_CHALLENGE&&this.getType()!=Constant.FIGHT_TYPE_STAKE)
    {
        ...
        if(energy<=0)
        {
          if(this.getType()==Constant.FIGHT_TYPE_AGRESSION&&fighter.getTraqued())
          {
            if(getTeam1().containsValue(fighter))
             new TimerWaiterPlus(() ->  player.teleportFaction(this.getAlignementOfTraquer(this.getTeam0().values(),player)),1500);
            else
             new TimerWaiterPlus(() ->  player.teleportFaction(this.getAlignementOfTraquer(this.getTeam0().values(),player)),1500);
            player.setEnergy(1);
          }
        ...
        else
        {
          ...
          else
          {
            if(player.getCurMap()!=null&&player.getCurMap().getSubArea()!=null&&(player.getCurMap().getSubArea().getId()==319||player.getCurMap().getSubArea().getId()==210))
            {
              ...
              new TimerWaiterPlus(() ->  player.teleportLaby((short)9558,224),1000);
              new TimerWaiterPlus(() -> {
                Minotoror.sendPacketMap(player); // Retarde le paquet sinon les portes sont fermï¿½s. Le paquet de GameInformation doit faire chier ce pï¿½dï¿½
                player.setPdv(1);
              },3500);
  ...
  public String getGE(int win)
  {
    ...
    try
    {
      ...
      if(mobCapturable&&!Capture.isInArenaMap(this.getMapOld().getId()))
      {
        ...
        this.setFullSoul(new Capture(Database.getStatics().getObjectData().getNextId(),1,7010,Constant.ITEM_POS_NO_EQUIPED,stats)); // Crï¿½e la pierre d'ï¿½me
      ...
      /*  int totalLoserRank=0;
        ...
        if(isAlone)
        {
          for(Fighter fighter : loosers)
          {
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(curPlayer,"Thomas Sacre: The contract has been executed, please come and see me to claim your reward.","000000");
      ...
      if(this.getType()==Constant.FIGHT_TYPE_STAKE)
      {
        ...
        {
          final Player player=fighter.getPersonnage();
          if(player!=null&&player.getStake()!=null)
            player.setStake(null);
        }
        ...
        for(final Fighter fighter : winners)
        {
          ...
          if(player!=null&&player.getStake()!=null)
          {
            stakeItems=player.getStake().winItems(player);
            stakeKamas=player.getStake().winKamas(player);
            player.setStake(null);
          }
        }
      }
        ...
        switch(this.getType())
        {
          ...
          case Constant.FIGHT_TYPE_PVM:
            try
            {
              ...
              { // mob loose..
                list=Fight.give(group.isFix() ? RespawnGroup.fixMobGroupObjects.get(this.getMapOld().getId()+","+group.getCellId()).getLeft() : group.getObjects(),winners);
      ...
      for(Fighter i : winners)
      {
        ...
        if(getType()==Constant.FIGHT_TYPE_PVT||getType()==Constant.FIGHT_TYPE_PVM||getType()==Constant.FIGHT_TYPE_CHALLENGE||getType()==Constant.FIGHT_TYPE_DOPEUL)
        {
          ...
          if(player!=null)
          {
            ...
            System.out.println(xpPlayer);
            if(VIP)xpPlayer = (long) (xpPlayer+(xpPlayer*0.25));
            System.out.println(xpPlayer);
            XP.set(xpPlayer);
          ...
          else
          {
            ...
            for(Drop drop : temporary3)
            {
              ...
              if(charPros<1)
                charPros=1;
              if(VIP)charPros = (int) (charPros+(charPros*0.25));
          ...
          if(this.getType()==Constant.FIGHT_TYPE_STAKE)
          {
            ...
            if(stakeItems!=null&&!stakeItems.isEmpty())
            {
              for(Pair<Integer, Integer> drop : stakeItems)
              {
                if(!drops.isEmpty())
                  drops+=",";
                drops+=World.getGameObject(drop.getLeft()).getTemplate().getId()+"~"+drop.getRight();
              }
            }
