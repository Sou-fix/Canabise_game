...
public class AstarPathfinding
{
  ...
  public ArrayList<GameCase> getShortestPath(int value)
  {
    ...
    while(!openList.isEmpty()&&(!closeList.containsKey(getCellEnd())))
    {
      ...
      if(nodeCurrent.getCellId()==getCellEnd()&&!PathFinding.cellArroundCaseIDisOccuped(getFight(),nodeCurrent.getCellId()))
        return getPath();
      ...
      for(int loc0=0;loc0<4;loc0++)
      {
        ...
        if(getMap().getCase(cell)==null)
          continue;
        if(!getMap().getCase(cell).isWalkable(true,true,-1)&&cell!=getCellEnd())
          continue;
        if(PathFinding.haveFighterOnThisCell(cell,getFight())&&cell!=getCellEnd())
          continue;
        ...
        if(openList.containsKey(cell))
        {
          if(openList.get(cell).getCountG()>getCostG(node))
          {
            ...
            openList.get(cell).setCountF(openList.get(cell).getCountG()+openList.get(cell).getHeristic());
          }
        } else
        {
          if(value==0)
